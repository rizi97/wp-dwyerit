<?php
/**
 * Template Name: Support template
 *
 * This is the template that displays all pages by default.
 * Please note that this is the WordPress construct of pages
 * and that other 'pages' on your WordPress site may use a
 * different template.
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package dwyerit
 */

get_header();

$download_link = get_field('download_link') ?? '';
?>

<style>
  body {
    overflow-x: hidden;
  }
</style>

<div class="page-wrapper">

  <section class="section wf-section">
    <div data-w-id="3dadc81d-a9a6-5722-3f7a-b86093dbc839"
      style="transform: translate3d(0px, 0px, 0px) scale3d(1, 1, 1) rotateX(0deg) rotateY(0deg) rotateZ(0deg) skew(0deg, 0deg); opacity: 1; transform-style: preserve-3d;"
      class="container-small">
      <div class="card confirm-email"><img
          src="https://assets.website-files.com/6368a1d253919b5df902c31c/6368a1d253919b7cad02c417_image-contact-saas-x-template.svg"
          loading="eager" alt="" class="image card-confirm-email-icon">
        <div class="card-confirm-email-content">
          <h1 class="title h2-size confirm-email">Remote Support</h1>
          <p class="paragraph confirm-email"><strong>This software allows us to help you from a distance. Before
              downloading, please call us on </strong><a href="tel:+12154887835"><strong>215 488 7835.</strong></a><a
              href="https://www.msplaunchpadtech.com/remote-support#"><strong><br></strong></a></p><a
            href="<?php echo esc_url($download_link); ?>" target="_blank"
            class="button-primary-2 full-width utility-page w-inline-block">
            <div class="button-primary-text-wrapper">
              <div>Download Remote Support Tool</div>
              <div class="button-primary-arrow w-embed"><svg width="100%" height="100%" viewBox="0 0 16 14" fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M16 6.97949C16 6.70703 15.877 6.43457 15.6748 6.23242L10.2256 0.791992C9.99707 0.563477 9.75098 0.466797 9.50488 0.466797C8.9248 0.466797 8.51172 0.879883 8.51172 1.4248C8.51172 1.71484 8.62598 1.96094 8.81055 2.14551L10.6914 4.03516L12.9238 6.08301L11.0342 5.97754H1.14648C0.540039 5.97754 0.118164 6.38184 0.118164 6.97949C0.118164 7.57715 0.540039 7.99023 1.14648 7.99023H11.0342L12.9238 7.87598L10.6914 9.92383L8.81055 11.8223C8.62598 11.998 8.51172 12.2441 8.51172 12.5342C8.51172 13.0879 8.9248 13.501 9.50488 13.501C9.75098 13.501 9.99707 13.3955 10.208 13.1846L15.6748 7.72656C15.877 7.5332 16 7.26074 16 6.97949Z"
                    fill="currentcolor"></path>
                </svg></div>
            </div>
          </a>
        </div>
      </div>
    </div>
    <div class="bg utility-page taller"></div>
  </section>

</div>

<?php

get_footer();